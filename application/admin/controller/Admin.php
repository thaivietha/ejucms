...
class Admin extends Base {
    ...
    public function login()
    {
        ...
        if (IS_POST) {
            ...
            if (!testWriteAble(DATA_PATH.'session')) {
                $this->error('请仔细检查以下问题：<br/>1、磁盘空间大小是否100%；<br/>2、站点目录权限是否为755；<br/>3、站点目录的权限，禁止用root:root ；<br/>4、如还没解决，请点击：<a href="http://www.ejucms.com/index.php?m=plugins&c=Ask&a=details&ask_id=9" target="_blank">查看教程</a>');
            }
    ...
    public function logout()
    {
        ...
        // session_destroy();
        session::clear();
        $this->success("安全退出", $this->request->baseFile());
    }
    ...
    public function ajax_settypeid(){
        ...
        if ($typeid){
            $admin_info = session('admin_info');
            $r = M('admin')->where("admin_id = ".$admin_info['admin_id'])->save(array('update_time'=>time(),'typeid'=>$typeid));
            if ($r){
