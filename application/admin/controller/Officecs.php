...
class Officecs extends Base
{
    ...
    public function index()
    {
        ...
        $count = $this->archives_db->alias('a')
            ->join("officecs_system c","a.aid=c.aid","LEFT")
        ...
        if ($list) {
            ...
            $row = $this->archives_db
                ...
                ->join("officecs_system c","a.aid = c.aid","LEFT")
                ->join("officecs_content d","a.aid = d.aid","LEFT")
                ->where('a.aid', 'in', $aids)
                ->getAllWithIndex('aid');
            foreach ($list as $key => $val) {
                ...
                empty($typeid) && $typeid = $row[$val['aid']]['typeid'];
            }
        }
...
//            }
            ...
            if ($aid) {
                ...
                // 生成静态页面代码
                $this->success("操作成功!", url("Index/uphtml").'&aid='.$aid.'&tid='.$post['typeid'].'&controller=Officecs&action=add');
                exit;
            }
    ...
    public function edit()
    {
        if (IS_POST) {
            ...
            if ($r) {
                ...
                // 生成静态页面代码
                $this->success("操作成功!", url("Index/uphtml").'&aid='.$data['aid'].'&tid='.$typeid.'&controller=Officecs&action=edit');
                exit;
            }
        ...
        //读取相册信息
        $photo_list = model("officecs_photo")->getListByWhere(['aid'=>$id,'is_del'=>0]);
