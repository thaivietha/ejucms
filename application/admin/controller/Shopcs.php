...
class Shopcs extends Base
{
    // 模型标识
    public $nid = 'shopcs';
    // 模型ID
    public $channeltype = 14;
    // 模型附加表
    public $table = 'Shopcs';
    // 模型信息
    ...
    public function index()
    {
        ...
        $count = $this->archives_db->alias('a')
            ->join("shopcs_system c","a.aid=c.aid","LEFT")
        ...
        if ($list) {
            ...
            $row = $this->archives_db
                ...
                ->join("shopcs_system c","a.aid = c.aid","LEFT")
                ->join("shopcs_content d","a.aid = d.aid","LEFT")
                ->where('a.aid', 'in', $aids)
                ->getAllWithIndex('aid');
...
//            }
            ...
            if ($aid) {
                ...
                // 生成静态页面代码
                $this->success("操作成功!", url("Index/uphtml").'&aid='.$aid.'&tid='.$post['typeid'].'&controller=Shopcs&action=add');
                exit;
            }
    ...
    public function edit()
    {
        if (IS_POST) {
            ...
            if ($r) {
                ...
                // 生成静态页面代码
                $this->success("操作成功!", url("Index/uphtml").'&aid='.$data['aid'].'&tid='.$typeid.'&controller=Shopcs&action=edit');
                exit;
            }
        ...
        //读取相册信息
        $photo_list = model("shopcs_photo")->getListByWhere(['aid'=>$id,'is_del'=>0]);
