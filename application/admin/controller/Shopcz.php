...
class Shopcz extends Base
{
    ...
    public function index()
    {
        ...
        $count = $this->archives_db->alias('a')
            ->join("shopcz_system c","a.aid=c.aid","LEFT")
        ...
        if ($list) {
            ...
            $row = $this->archives_db
                ...
                ->join("shopcz_system c","a.aid = c.aid","LEFT")
                ->join("shopcz_content d","a.aid = d.aid","LEFT")
                ->where('a.aid', 'in', $aids)
                ->getAllWithIndex('aid');
...
//            }
            ...
            if ($aid) {
                ...
                // 生成静态页面代码
                $this->success("操作成功!", url("Index/uphtml").'&aid='.$aid.'&tid='.$post['typeid'].'&controller=Shopcz&action=add');
                exit;
            }
    ...
    public function edit()
    {
        ...
        if (IS_POST) {
            ...
            if ($r) {
                ...
                // 生成静态页面代码
                $this->success("操作成功!", url("Index/uphtml").'&aid='.$data['aid'].'&tid='.$typeid.'&controller=Shopcz&action=edit');
                exit;
            }
        ...
        /*--end*/
        //读取相册信息
        $photo_list = model("shopcz_photo")->getListByWhere(['aid'=>$id,'is_del'=>0]);
