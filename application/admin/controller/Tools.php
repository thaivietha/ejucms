...
class Tools extends Base {
            ...
            if(false === $start){ //出错
                ...
                if(isset($tables[++$id])){
                    ...
                    if (is_dir($install_path) && file_exists($install_path)) {
                        ...
                        $dstfile = $install_path.'/ejucms.sql';
                        if(@copy($srcfile, $dstfile)){
                            /*替换所有表的前缀为官方默认eju_，并重写安装数据包里*/
                            $eyouDbStr = file_get_contents($dstfile);
                            $dbtables = Db::query('SHOW TABLE STATUS');
                            foreach ($dbtables as $k => $v) {
                                $tableName = $v['Name'];
                                if (preg_match('/^'.PREFIX.'/i', $tableName)) {
                                    $eyTableName = preg_replace('/^'.PREFIX.'/i', 'eju_', $tableName);
    ...
    public function optimize()
    {
        if (IS_AJAX_POST) {
            ...
            $this->success("操作成功！", url('Tools/index'));
        }
        $this->error("操作失败！");
    }
