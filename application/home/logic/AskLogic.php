...
// 查询条件处理
    public function GetAskWhere($param = array(), $users_id = null)
    {
        ...
        // 创始人可以看到所有数据，包括未审核问题
//        if (0 != $users_id) unset($where['a.is_review']);
        //指定楼盘
    ...
    public function GetUrlData($param = array(), $SpecifyUrl = null)
    {
        ...
        // ROOT_DIR.'/index.php?m=home&c=Ask&a=add_ask';

        // 编辑问题url
        $result['EditAskUrl'] = url('home/Ask/edit_ask', ['ask_id'=>$param['ask_id']]);
        // ROOT_DIR.'/index.php?m=home&c=Ask&a=edit_ask&ask_id='.$param['ask_id'];

        // 用户问题首页
        $result['UsersIndexUrl'] = url('home/Ask/ask_index');
        // ROOT_DIR.'/index.php?m=home&c=Ask&a=ask_index';

        // 编辑回答url
        $result['EditAnswer'] = url('home/Ask/ajax_edit_answer');
        // ROOT_DIR.'/index.php?m=home&c=Ask&a=ajax_edit_answer';
        ...
        // ROOT_DIR.'/index.php?m=home&c=Ask&a=ajax_best_answer&ask_id='.$param['ask_id'];

        // 获取指定数量的评论数据（分页）
        $result['ShowCommentUrl'] = url('home/Ask/ajax_show_comment', ['ask_id'=>$param['ask_id'], '_ajax'=>1], true, false, 1, 1, 0);
        // ROOT_DIR.'/index.php?m=home&c=Ask&a=ajax_show_comment&ask_id='.$param['ask_id'].'&_ajax=1';

        // 创始人审核评论URL(前台)
        $result['ReviewCommentUrl'] = url('home/Ask/ajax_review_comment', ['ask_id'=>$param['ask_id'], '_ajax'=>1], true, false, 1, 1, 0);
        // ROOT_DIR.'/index.php?m=home&c=Ask&a=ajax_review_comment&ask_id='.$param['ask_id'].'&_ajax=1';

        // 创始人审核问题URL(前台)
        $result['ReviewAskUrl'] = url('home/Ask/ajax_review_ask', ['_ajax'=>1], true, false, 1, 1, 0);
        // ROOT_DIR.'/index.php?m=home&c=Ask&a=ajax_review_ask&_ajax=1';

        // 按点赞量排序url
        $result['AnswerLikeNum'] = url('home/Ask/details', ['ask_id' => $param['ask_id']], true, false, 1, 1, 0);

        // 等待回答url
        if (!empty($param['type_id'])) {
            $result['PendingAnswerUrl'] = url('home/Ask/index', ['type_id'=>$param['type_id'], 'is_recom'=>2]);
            // ROOT_DIR.'/index.php?m=home&c=Ask&a=index&type_id='.$param['type_id'].'&is_recom=2';
        }
