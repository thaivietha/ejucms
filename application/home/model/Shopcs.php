    ...
    public function getInfo($aid, $field = '', $isshowbody = true)
    {
        ...
        if ($isshowbody) {
            ...
            $result = db('archives')->field($field)
                ->alias('a')
                ->join('__SHOPCS_CONTENT__ b', 'b.aid = a.aid', 'LEFT')
                ->join('__ARCTYPE__ c', 'a.typeid = c.id', 'LEFT')
                ->join('__SHOPCS_SYSTEM__ d', 'a.aid = d.aid', 'LEFT')
                ->find($aid);
        } else {
...
//        $param_query['tid'] = $this->tid;
        ...
        $lists = db('archives')
            ...
            ->join("shopcs_content c","a.aid = c.aid",'LEFT')
            ->join("shopcs_system d","a.aid = d.aid",'LEFT')
            ->where($where)
            ->order($sort)
            ->select();
        if($lists)
        {
            ...
            foreach ($lists as $key => $val) {
                ...
                if ($val['is_jump'] == 1) {
                    ...
                    $lists[$key]['arcurl'] = arcurl("home/Shopcs/view", $val);
                }
