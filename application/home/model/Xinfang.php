...
//        $param_query['tid'] = $this->tid;
        ...
        if($lists)
        {
            foreach ($lists as $key => $val) {
                ...
                $lists[$key]['characteristic_name_i'] = $lists[$key]['characteristic_name'] = '';
...
//            }
     ...
     * 获取区域房源整合
     */
    private function countAreaHouse($lists,$city){
        $area_arr = model('region')->getAll($city,'*','id');
        $data = [];
        foreach ($lists as $val){
            if (empty($data[$val['area_id']])){
                $data[$val['area_id']] = [
                    'total_price' =>$val['average_price'],
                    'price' => $val['average_price'],
                    'city_name' => !empty($area_arr[$val['area_id']]) ? $area_arr[$val['area_id']]['name'] : "",
                    'total'     => 1,
                    'lat'       => !empty($area_arr[$val['area_id']]) ? $area_arr[$val['area_id']]['lat'] : "",
                    'lng'       => !empty($area_arr[$val['area_id']]) ? $area_arr[$val['area_id']]['lng'] : "",
                ];
            }else{
                $data[$val['area_id']]['total_price'] += $val['average_price'];
                $data[$val['area_id']]['total'] += 1;
                $data[$val['area_id']]['price'] = ceil($data[$val['area_id']]['total_price']/$data[$val['area_id']]['total']);
            }
        }
