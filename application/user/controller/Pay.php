...
class Pay extends Base
{
    ...
    public function _initialize() {
        ...
        $pay_wechat = unserialize($this->usersConfig['pay_wechat_config']);
        $pay_alipay = unserialize($this->usersConfig['pay_alipay_config']);
        if((empty($pay_wechat) && empty($pay_alipay)) || (!empty($pay_wechat['is_open_wechat'])  && !empty($pay_alipay['is_open_alipay']))){
            $this->error('支付功能尚未开启！',  url('user/Users/index'));
            exit;
        }
        // 是否为手机端
        $is_mobile = 2;     // 其他端
    ...
    public function pay_consumer_details()
    {
        ...
        model('Pay')->UpdateOrderData($this->users_id,$this->usersConfig['users_order_validity']);
    ...
    public function get_alipay_order()
    {
        if (IS_AJAX_POST) {
            ...
            if($transaction_type == 2){
                ...
                $Result =$this->users_money_db->where($where)->find();
                if (empty($Result)) $this->error('订单信息错误，请尝试刷新');
            }
            // 引入文件
            ...
            // 拼装配置
            $config['version']     = $pay_alipay_config['version'];
            ...
            if(!empty($unified_number)){
                ...
                if ($result['return_code'] == 'SUCCESS' && $result['result_code'] == 'SUCCESS'){
                    if ($result['trade_state'] == 'SUCCESS' && !empty($result['transaction_id'])) {
                        ...
                        // 付款中
                        $this->success('正在付款中1~~~~', '', ['status'=>0]);
                    }
                }else{
                    $msg = '订单号：'.$unified_number.'，正在付款中2~~~~~';
