...
class Officecs extends Model
{
    ...
    public function afterSave($aid, $post, $opt)
    {
        ...
        if (!empty($post['photo_title'])){
            ...
            model('officecs_photo')->where("aid","=",$aid)->whereNotIn('photo_id',$notdel_data)->delete();
            $insert_data && model('officecs_photo')->insertAll($insert_data);
            $update_data && model('officecs_photo')->saveAll($update_data);
        }
        //处理价格变动
        $price = model('officecs_price')->getLast($aid,"*","type");
        ...
        if ($price){  //之前已经存在价格记录
            if ((!empty($average_price) && $price[1]['price'] != $average_price) || (!empty($total_price) && $price[3]['price'] != $total_price)){
                ...
                if ($price[1]['create_time'] >= $beginToday && $price[1]['create_time'] <= $endToday){  //今天之内存在变动，更新
                    ...
                    model('officecs_price')->saveAll($price);
                }else{  //今天之前存在更新，插入
                    ...
                    $price_insert[] = ['aid'=>$aid,'price'=>$total_price,'type'=>3,'create_time'=>time(),
                        'day'=>date('Y-m-d',time()), 'month'=>date('Y-m',time()), 'year'=>date('Y',time())];
                    model('officecs_price')->insertAll($price_insert);
                }
            }
        }else{
            $price_insert[] = ['aid'=>$aid,'price'=>$average_price,'type'=>1,'create_time'=>time(),
    ...
    public function afterDel($aidArr = array())
    {
        ...
        // 同时删除内容
        M('officecs_content')->where(array('aid'=>array('IN', $aidArr)))->delete();
        // 同时删除中间表
        M('officecs_system')->where(array('aid'=>array('IN', $aidArr)))->delete();
        // 同时删除相册表
        M('officecs_photo')->where(array('aid'=>array('IN', $aidArr)))->delete();
        // 同时删除TAG标签
    ...
    public function getOne($condition,$fields = "d.*,c.*,b.*, a.*, a.aid as aid,d.total_price as price"){
        $row = db('archives')
            ...
            ->join('officecs_content c','a.aid = c.aid')
