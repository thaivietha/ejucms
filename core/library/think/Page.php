...
class Page{
    ...
    private $config  = array(
        'header' => '<span class="rows">共 %TOTAL_ROW% 条数据</span>',
        /*
        'prev'   => '<<',
        'next'   => '>>',
        'first'  => '1...',
...
//        $this->nowPage    = empty($_REQUEST[$this->p]) ? 1 : intval($_REQUEST[$this->p]);        
    ...
    public function show() {
        ...
        if (1 < $this->totalPages) {
            ...
            $up_page = $up_row > 0 ? '<li id="example1_previous" class="paginate_button previous disabled"><a class="prev" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a></li>' : '<li id="example1_previous" class="paginate_button previous disabled"><a class="prev" href="javascript:void(0);" style="cursor: unset;">' . $this->config['prev'] . '</a></li>';
        }
        ...
        if (1 < $this->totalPages) {
            ...
            $down_page = ($down_row <= $this->totalPages) ? '<li id="example1_next" class="paginate_button next"><a class="next" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a></li>' : '<li id="example1_next" class="paginate_button next"><a class="next" href="javascript:void(0);" style="cursor: unset;">' . $this->config['next'] . '</a></li>';
        }
...
//                    $link_page .= '<span class="current">' . $page . '</span>';
        ...
        if (!trim($page_str)) {
            ...
            $page_str = "<div class='dataTables_paginate paging_simple_numbers'><ul class='pagination mt10'>{$page_str}</ul></div>";
        }
        return $page_str;
    }
}
