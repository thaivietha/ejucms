...
class Template
{
    ...
    protected $config = [
        ...
        'taglib_pre_load'    => 'eju', // 需要额外加载的标签库(须指定标签库名称)，多个以逗号分隔
    ...
    public function parse(&$content)
    {
        ...
        // 检查eju:include语法  by 小虎哥
        $this->parseEjuInclude($content);
        // 检查include语法
    ...
    private function parsePhp(&$content)
    {
        ...
        if (isset($view_replace_str['__EVAL__'])) {
            if (stristr($content, '{eju:php}')) { // 针对{eju:php}标签语法处理
                preg_match_all('/{eju\:php}.*{\/eju\:php}/iUs', $content, $matchs);
                $matchs = !empty($matchs[0]) ? $matchs[0] : [];
                if (!empty($matchs)) {
                    foreach($matchs as $key => $val){
                        $valNew = preg_replace('/{(\/)?eju\:php}/i', '', $val);
                        $valNew = preg_replace("/([\W]+)eval(\s*)\(/i", 'intval(', $valNew);
                        $valNew = preg_replace("/^eval(\s*)\(/i", 'intval(', $valNew);
                        $valNew = "{eju:php}{$valNew}{/eju:php}";
    ...
    /**
     * 解析模板中的eju:include标签 by 小虎哥
     * @access private
     * @param  string $content 要解析的模板内容
     * @return void
     */
    private function parseEjuInclude(&$content)
    {
        $regex = $this->getRegex('eju:include');
            ...
            if (preg_match_all($regex, $template, $matches, PREG_SET_ORDER)) {
                foreach ($matches as $match) {
                    ...
                    if (preg_match('/^([^\\/]+)(.*)\.'.$this->config['view_suffix'].'$/i', $file) === 1) {
                        // 支持模板引入的写法： {eju:include file="header.htm" /} 或者 {eju:include file="header" /}  或者 {eju:include file="users/header.htm" /} 或者 {eju:include file="users/header" /}
                        $file = str_replace('.'.$this->config['view_suffix'], '', $file);
                    } else if (stristr($file, '.'.$this->config['view_suffix']) && (stristr($file, '/') || stristr($file, '\\'))) {
                        // 支持绝对路径的模板引入写法： {eju:include file="/template/pc/header.htm" /}
                        $file  = '.'.$file;
                    }
                    unset($array['file']);
                    // 分析模板文件名并读取内容
