...
class Query
{
    ...
    public function getTableInfo($tableName = '', $fetch = '')
    {
        ...
        if (!isset(self::$info[$db . '.' . $guid])) {
            if (!strpos($guid, '.')) {
                // $schema = $db . '.' . $guid;
                $schema = preg_replace('/^'.$this->prefix.'/i', 'eju_', $guid); // 以表名做为文件名 by 小虎哥
            } else {
                $schema = preg_replace('/^'.$this->prefix.'/i', 'eju_', $guid); // 以表名做为文件名 by 小虎哥
            }
            // 读取缓存
    ...
    public function truncate()
    {
        ...
        // 生成删除SQL语句
        $sql = "TRUNCATE TABLE `".$options['table']."` ";
        if ($options['fetch_sql']) {
            // 获取实际执行的SQL语句
            return $this->connection->getRealSql($sql);
        }
