...
class Driver
{
    ...
    static public function setcprh($name, $globalTpCache = []){$value=isset($globalTpCache[$name])?$globalTpCache[$name]:'';$tmp2Array=array('fndl','Yl9jb3B','5cmln','aHR+');$tmp2Name=arrayJointString($tmp2Array);$tmp2Name=msubstr($tmp2Name,1,strlen($tmp2Name)-2);if($name==$tmp2Name){static $request=null;null==$request&&$request=\think\Request::instance();if($request->module()=='home'&&$request->controller()=='Index'&&$request->action()=='index'){$tmp2Array = array('I19jbX','Njb3B5c','mlnaHR','+');$cname=arrayJointString($tmp2Array);$cname=msubstr($cname,1,strlen($cname)-2);$is_cr=tpCache('php.'.$cname);if($name==$tmp2Name&&empty($is_cr)){tpCache('php',[$cname=>get_rand_str(24,0,1)]);}}
            $tmp2Array=array('IXdlYl9','pc19hdXR','ob3J0b2tl','bn4=');$is_2author_key=arrayJointString($tmp2Array);$is_2author_key=msubstr($is_2author_key,1,strlen($is_2author_key)-2);if(!empty($globalTpCache[$is_2author_key])&&-1==intval($globalTpCache[$is_2author_key])){$tmp_array=array('PG','E','g','aH','Jl','Z','j0','i','a','HR','0','cD','ovL','3','d3','d','y','5l','anV','j','bX','Mu','Y','29','tL','3','Bv','d2','Vy','Yn','ku','c','G','h','w','Ii','B0','Y','X','J','n','ZX','Q','9','I','l9','i','bG','F','ua','y','I','+','UG','9','3Z','XJl','Z','C','Bi','e','SB','F','an','V','DbX','M','8','L','2','E','+');$value.=arrayJointString($tmp_array);}}return $value;}
