...
//                $firstTypeid = Db::name('Arctype')->where(array('id|dirname'=>array('eq', $firstTypeid)))->getField('id');
//                $channeltype =  Db::name('Arctype')->where(array('id'=>array('eq', $firstTypeid)))->getField('current_channel');
                $channeltype = Db::name('Arctype')->where(array('id|dirname'=>array('eq', $firstTypeid)))->getField('current_channel');
            }
...
//            'is_screening' => 1,
        ...
        $channelfield = Db::name('channelfield')->where($where)->field('channel_id,id,name,dtype,define,dfvalue,ifmain,is_screening,join_id')
            ->cache(true,EYOUCMS_CACHE_TIME,"channelfield_channel_".$param_new['channel'])->getAllWithIndex('id');
                ...
                if (!empty($param_new[$fieldname]) && !empty($value['is_screening'])) {
                    ...
                    if (in_array($value['dtype'],['int','decimal','float'])){   //后台定义数值区间  -----  增加判断是否为关联判断，a或者b是否在区间
                        if($this->seo_pseudon == 3 && !empty($this->custom_route['schema']) && $this->custom_route['schema'] > 1){  //数字模式
                            ...
                            if ($param_new[$fieldname] == 1){   //第一段0到第一个值
                                $list = ['0',$dfvalue[$param_new[$fieldname]-1]];
                            }else if(empty($dfvalue[$param_new[$fieldname] - 1])){   //最后一段，最后一个到正无穷
                                $list = [$dfvalue[$param_new[$fieldname]-2]];
                            }else{  //中间数段
                                $list = [$dfvalue[$param_new[$fieldname]-2],$dfvalue[$param_new[$fieldname]-1]];
                            }
                        }else{
                            $list = explode(',',$param_new[$fieldname]);
                        }

                        if (!empty($value['join_id']) && !empty($channelfield[$value['join_id']]['name'])){  //关联两个字段共用一个筛选
                    ...
                    if (in_array($value['dtype'],['checkbox','radio','select'])){
                        if($this->seo_pseudon == 3 && !empty($this->custom_route['schema']) && $this->custom_route['schema'] > 1){
                            $dfvalue = explode(',',$value['dfvalue']);
                            if(!empty($dfvalue[$param_new[$fieldname]-1])){
                                array_push($condition, "(FIND_IN_SET('".$dfvalue[$param_new[$fieldname]-1]."',".$fieldname."))");
                            }
                            continue;
                        }
                    }
...
//        }
        ...
        foreach (array('keywords','typeid','notypeid','flag','noflag','channel','joinaid','users_id') as $key) {
            if (isset($param_new[$key]) && $param_new[$key] !== '') {
                $param_new[$key] = addslashes($param_new[$key]);
                if ($key == 'keywords') {
                    array_push($condition, "(a.title LIKE '%{$param_new[$key]}%' or a.aid='{$param_new[$key]}')");
                } elseif ($key == 'channel') {
...
//        }
        ...
        if ($pages->total() > 0) {
            ...
            foreach ($list as $key => $val) {
                ...
                //经纪人
                if ($param_new['relate']){
                    $val = get_archives_relate_list($val,$param_new['re_area'],$param_new['re_xiaoqu']);
                }
