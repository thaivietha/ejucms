/*! Zepto 1.2.0 (generated by grunt) - zepto event ajax form ie data touch fx_methods fx - zeptojs.com/license */
var Zepto = function () {
	function a(a) {
	...
	return X.matches = function (a, b) {
		...
		if (!a) return X.Z();
		if ("string" == typeof a)
			if (a = a.trim(), "<" == a[0] && K.test(a)) d = X.fragment(a, RegExp.$1, c), a = null;
			else {
			...
			else if (K.test(a)) d = X.fragment(a.trim(), RegExp.$1, c), a = null;
			else {
				if (c !== w) return y(c).find(a);
				d = X.qsa(G, a)
			}
		...
		is: function (a) {
			return "string" == typeof a ? this.length > 0 && X.matches(this[0], a) : a && this.selector == a.selector
		},
		not: function (a) {
			var c = [];
		...
		prop: function (a, b) {
			return a = Z[a] || a, "string" != typeof a || 1 in arguments ? this.each(function (c) {
				if (e(a))
					for (x in a) this[Z[x] || x] = a[x];
				else this[a] = r(this, b, c, this[a])
			}) : this[0] && this[0][a]
		},
		...
		offset: function (a) {
			...
			if (G.documentElement !== this[0] && !y.contains(G.documentElement, this[0])) return {
				top: 0,
				left: 0
			};
		...
		y.fn[b] = function () {
			var b, e, f = y.map(arguments, function (c) {
					...
					return b = a(c), "array" == b ? (c.forEach(function (a) {
						return a.nodeType !== w ? d.push(a) : y.zepto.isZ(a) ? d = d.concat(a.get()) : void(d = d.concat(X.fragment(a)))
...
window.Zepto = Zepto, void 0 === window.$ && (window.$ = Zepto),
	function (a) {
		...
		function j(b, c) {
			if (c || !b.isDefaultPrevented) {
				...
				} catch (d) {}(c.defaultPrevented !== l ? c.defaultPrevented : "returnValue" in c ? c.returnValue === !1 : c.getPreventDefault && c.getPreventDefault()) && (b.isDefaultPrevented = v)
			}
			return b
		}
	...
	function (a) {
		...
		function c(a, c, d, e) {
			return a.global ? b(c || t, d, e) : void 0
		}
		...
		var q, r, s = +new Date,
			...
			z = /^\s*$/,
			A = t.createElement("a");
		A.href = window.location.href, a.active = 0, a.ajaxJSONP = function (b, c) {
			if (!("type" in b)) return a.ajax(b);
			var d, e, i = b.jsonpCallback,
				j = (a.isFunction(i) ? i() : i) || "Zepto" + s++,
			...
			return c && c.promise(n), a(k).on("load error", function (f, i) {
				...
				d = arguments
			}, k.src = b.url.replace(/\?(.+)=\?/, "?$1=" + j), t.head.appendChild(k), b.timeout > 0 && (e = setTimeout(function () {
				m("timeout")
			}, b.timeout)), n)
		}, a.ajaxSettings = {
			type: "GET",
			...
			var c, e, i = a.extend({}, b || {}),
				o = a.Deferred && a.Deferred();
			for (q in a.ajaxSettings) void 0 === i[q] && (i[q] = a.ajaxSettings[q]);
			d(i), i.crossDomain || (c = t.createElement("a"), c.href = i.url, c.href = c.href, i.crossDomain = A.protocol + "//" + A.host != c.protocol + "//" + c.host), i.url || (i.url = window.location.toString()), (e = i.url.indexOf("#")) > -1 && (i.url = i.url.slice(0, e)), n(i);
			var p = i.dataType,
				s = /\?.+=\?/.test(i.url);
			if (s && (p = "jsonp"), i.cache !== !1 && (b && b.cache === !0 || "script" != p && "jsonp" != p) || (i.url = m(i.url, "_=" + Date.now())), "jsonp" == p) return s || (i.url = m(i.url, i.jsonp ? i.jsonp + "=?" : i.jsonp === !1 ? "" : "callback=?")), a.ajaxJSONP(i, o);
			var u, v = i.accepts[p],
				w = {},
				x = function (a, b) {
					w[a.toLowerCase()] = [a, b]
				...
				B = i.xhr(),
				C = B.setRequestHeader;
			if (o && o.promise(B), i.crossDomain || x("X-Requested-With", "XMLHttpRequest"), x("Accept", v || "*/*"), (v = i.mimeType || v) && (v.indexOf(",") > -1 && (v = v.split(",", 2)[0]), B.overrideMimeType && B.overrideMimeType(v)), (i.contentType || i.contentType !== !1 && i.data && "GET" != i.type.toUpperCase()) && x("Content-Type", i.contentType || "application/x-www-form-urlencoded"), i.headers)
				for (r in i.headers) x(r, i.headers[r]);
			if (B.setRequestHeader = x, B.onreadystatechange = function () {
					if (4 == B.readyState) {
						B.onreadystatechange = k, clearTimeout(u);
						var b, c = !1;
						if (B.status >= 200 && B.status < 300 || 304 == B.status || 0 == B.status && "file:" == y) {
							if (p = p || l(i.mimeType || B.getResponseHeader("content-type")), "arraybuffer" == B.responseType || "blob" == B.responseType) b = B.response;
							else {
								b = B.responseText;
								try {
									b = j(b, p, i), "script" == p ? (1, eval)(b) : "xml" == p ? b = B.responseXML : "json" == p && (b = z.test(b) ? null : a.parseJSON(b))
								} catch (d) {
									c = d
								}
								if (c) return h(c, "parsererror", B, i, o)
							}
							g(b, B, i, o)
						} else h(B.statusText || null, B.status ? "error" : "abort", B, i, o)
					}
				}, f(B, i) === !1) return B.abort(), h(null, "abort", B, i, o), B;
			var D = "async" in i ? i.async : !0;
			if (B.open(i.type, i.url, D, i.username, i.password), i.xhrFields)
				for (r in i.xhrFields) B[r] = i.xhrFields[r];
			for (r in w) C.apply(B, w[r]);
			return i.timeout > 0 && (u = setTimeout(function () {
				B.onreadystatechange = k, B.abort(), h(null, "timeout", B, i, o)
			}, i.timeout)), B.send(i.data ? i.data : null), B
		}, a.get = function () {
			...
			return g.length > 1 && (h.url = g[0], e = g[1]), h.success = function (b) {
				f.html(e ? a("<div>").html(b.replace(u, "")).find(e) : b), i && i.apply(f, arguments)
			}, a.ajax(h), this
		};
		...
		a.param = function (b, c) {
			...
			return d.add = function (b, c) {
				a.isFunction(c) && (c = c()), null == c && (c = ""), this.push(B(b) + "=" + B(c))
	...
	function (a) {
		...
		function c() {
			m = null, s.last && (s.el.trigger("longTap"), s = {})
		}
		...
		function e() {
			j && clearTimeout(j), k && clearTimeout(k), l && clearTimeout(l), m && clearTimeout(m), j = k = l = m = null, s = {}
		}
		...
		function h() {
			u && (a(document).off(r.down, o).off(r.up, p).off(r.move, q).off(r.cancel, e), a(window).off("scroll", e), e(), u = !1)
		}

		function i(i) {
			var v, w, x, y, z = 0,
				A = 0;
			h(), r = i && "down" in i ? i : "ontouchstart" in document ? {
				down: "touchstart",
				up: "touchend",
				move: "touchmove",
				cancel: "touchcancel"
			...
			} : !1, r && ("MSGesture" in window && (n = new MSGesture, n.target = document.body, a(document).bind("MSGestureEnd", function (a) {
				var b = a.velocityX > 1 ? "Right" : a.velocityX < -1 ? "Left" : a.velocityY > 1 ? "Down" : a.velocityY < -1 ? "Up" : null;
				b && (s.el.trigger("swipe"), s.el.trigger("swipe" + b))
			})), o = function (b) {
				(!(y = g(b, "down")) || f(b)) && (x = y ? b : b.touches[0], b.touches && 1 === b.touches.length && s.x2 && (s.x2 = void 0, s.y2 = void 0), v = Date.now(), w = v - (s.last || v), s.el = a("tagName" in x.target ? x.target : x.target.parentNode), j && clearTimeout(j), s.x1 = x.pageX, s.y1 = x.pageY, w > 0 && 250 >= w && (s.isDoubleTap = !0), s.last = v, m = setTimeout(c, t), n && y && n.addPointer(b.pointerId))
			}, q = function (a) {
				(!(y = g(a, "move")) || f(a)) && (x = y ? a : a.touches[0], d(), s.x2 = x.pageX, s.y2 = x.pageY, z += Math.abs(s.x1 - s.x2), A += Math.abs(s.y1 - s.y2))
			}, p = function (c) {
				(!(y = g(c, "up")) || f(c)) && (d(), s.x2 && Math.abs(s.x1 - s.x2) > 30 || s.y2 && Math.abs(s.y1 - s.y2) > 30 ? l = setTimeout(function () {
					s.el && (s.el.trigger("swipe"), s.el.trigger("swipe" + b(s.x1, s.x2, s.y1, s.y2))), s = {}
				}, 0) : "last" in s && (30 > z && 30 > A ? k = setTimeout(function () {
					var b = a.Event("tap");
					b.cancelTouch = e, s.el && s.el.trigger(b), s.isDoubleTap ? (s.el && s.el.trigger("doubleTap"), s = {}) : j = setTimeout(function () {
						j = null, s.el && s.el.trigger("singleTap"), s = {}
					}, 250)
				}, 0) : s = {}), z = A = 0)
			}, a(document).on(r.up, p).on(r.down, o).on(r.move, q), a(document).on(r.cancel, e), a(window).on("scroll", e), u = !0)
		}
	...
	function (a, b) {
		...
		var e = (window.document, a.fn.show),
			f = a.fn.hide,
			g = a.fn.toggle;
		a.fn.show = function (a, d) {
			return e.call(this), a === b ? a = 0 : this.css("opacity", 0), c(this, a, 1, "1,1", d)
		}, a.fn.hide = function (a, c) {
			return a === b ? f.call(this) : d(this, a, "0,0", c)
